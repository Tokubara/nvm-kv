...
RetCode EngineRace::read_index_file(int bucket_id, int &fd) { // fd并不会改变, 我觉得这里存const fd也行
    ssize_t rd;
    int sz;
    char buf[1024];
...
RetCode EngineRace::Open(const std::string &name, Engine **eptr) {
    *eptr = NULL;
    ...
    for (size_t i = 0; i < thread_cnt; i++) {
        ...
        memset(ptr, 0, chunckSize); // 这与read_index_file中的while ((rd = read(fd, (char *)&sz, 4)) > 0)是类似的
        log(std::to_string(i) + ": mmap loaded ");
        engine_race->mem_tmp[i] = (char *)ptr; //? 怎么存的是char*类型, 而不是更规整的类型? 难道是因为, 它的每一个大小并不一样? 你看, 还存了key_size, 大小不确定
...
EngineRace::~EngineRace() {
    log("Saving index");
    for (size_t i = 0; i < thread_cnt; i++) {
        munmap(mem_tmp[i], alloc_size_tmp[i]); // 因为的确用不着了, 信息都在index中
        int fd_index_save = open((dir_name + "/index_" + std::to_string(i)).c_str(), O_RDWR | O_CREAT, 0644); // 这个是重复写入的, 那么析构的开销挺大的
        for (auto &it : index[i]) {
        ...
        fsync(fd_index_tmp[i]); // wait, or can't be removed //? 我觉得不用
...
// 需要维护的是2部分: index数组, 还有写文件, 不需要直接write, 是通过mmap的方式, memcpy就行了
RetCode EngineRace::Write(const PolarString &key, const PolarString &value) {
    // static int write_cnt = 0;
    size_t bucket_id = calculate_bucket_id(key.ToString()); // calculate_bucket_id这个函数已经%thread_num了
    pthread_mutex_lock(mu_ + bucket_id);

    nextLoc[bucket_id].len = value.size(); // nextLoc的len部分, 直接写, 没有保存的意义, 有持续维护意义的是offset
    ...
    memcpy(mem_tmp[bucket_id] + mem_offset[bucket_id], (char *)&s, sizeof(s)); // 这是通过mmap来写的
    mem_offset[bucket_id] += sizeof(s); // 由此可知mem_offset, mem_tmp就是为了配合mmap而存在的
    ...
    nextLoc[bucket_id].offset += nextLoc[bucket_id].len; // 这里才是设置offset的地方
...
size_t EngineRace::read_data_file(size_t bucket_id, Location &loc, char *buf) {
    ...
    lseek(fd_data[bucket_id], 0, SEEK_END); // 感觉这里的回复是多余的
    return rd;
}
/*
